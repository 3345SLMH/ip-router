...
export function buildGetDTMF(files, options, call) {
    const name = options?.valName || 'dataGet';
    const max = options?.max ?? options?.max_digits ?? '';
    const min = options?.min ?? options?.min_digits ?? 1;
    const timeout = options?.timeout ?? options?.sec_wait ?? 7;
    const confirmType = options?.confirmType ?? options?.typing_playback_mode ?? 'digits';
    const setMusic = options?.setMusic ?? (options?.set_music ? 'yes' : 'no');
...
export function buildRecord(files, options, call) {
    const name = options?.valName || 'myRecord';
    return {
        type: 'record',
        name,
