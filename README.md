...
‫מכסה גם מקרי קצה שלא התקבלה מאיי פי הקריאה עם hangup=yes.<br>
...
ניתן לכתוב נתיב יחסי לשלוחה הנוכחית או לשלוחה הראשית
...
מתודה להעברת השיחה למערכת אחרת באיי פי המשיח ללא עלות יחידות, באמצעות "ראוטינג איי פי".

הפונקציה מקבלת ארגומנט יחיד - סטרינג של מספר מערכת באיי פי להעברת השיחה.
...
במתודה זו יש להעביר את הסטרינג בדיוק כפי שמעוניינים שהשרת של איי פי יקבל אותו, והוא לא עובר ולידציה או עיבוד.
...
מכיל את כל הפרמטרים שנשלחו מאיי פי -
...
- ברירות מחדל של המערכת - שהן כמו ברירות המחדל של איי פי ([defaults.js](lib/defaults.js))
- רמת ראוטר
- רמת שיחה
...
כל אפשרות מקבלת קדאיי פי ודורסת את זו שלפניה.
...
בעת החזרת תשובה שאינה `read` ולא גורמת לשרת של איי פי לחזור לראוטר - `id_list_message` או `go_to_folder`,
