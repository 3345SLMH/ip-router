...
interface Defaults {
     ...
     * @see [router.events](https://github.com/3345SLMH/ip-router#events) - קבלת קאלבק על שיחה חדשה/קבלת נתון ממאזין/ניתוק שיחה, למשתמשים מתקדמים. לא תלוי בהגדרה זו
     * @default false
     */
    printLog?: boolean
    /**
     ...
     * באם מוגדרת הסרה ‫(`true`) התווים יוסרו מהתשובה שתוחזר לאיי פי ולא תיזרק שגיאה<br>
     * ניתן להגדיר ערך זה ב3 רמות, ראו [תיעוד מלא](https://github.com/3345SLMH/ip-router#%D7%AA%D7%95%D7%95%D7%99%D7%9D-%D7%9C%D7%90-%D7%97%D7%95%D7%A7%D7%99%D7%99%D7%9D-%D7%91%D7%94%D7%A7%D7%A8%D7%90%D7%AA-%D7%98%D7%A7%D7%A1%D7%98)
     * @default false
     */
    ...
    read?: {
         ...
         * ‫מכסה גם מקרי קצה שלא התקבלה מאיי פי אות ניתוק (קריאה עם `hangup=yes` בפרטרים)<br>
         * 
...
type ipRouterOptions = ExpressRouterOptions & {
    /**
     * ‫ זמן המתנה לקבלת נתון מהמשתמש (במילישניות). במידה ולא התקבל הנתון בזמן הנ"ל, השיחה תימחק מה`activeCalls` וגם אם המשתמש יקיש השרת יקבל אותו כשיחה חדשה<br>
     * ‫מקבל מספר מילישיניות, או מחרוזת הקבילה ע"י ספריית [ספריית ms](https://npmjs.com/ms).<br>
     * ‫מכסה גם מקרי קצה שלא התקבלה מאיי פי אות ניתוק (קריאה עם `hangup=yes` בפרטרים)<br>
     ...
     * ראה דוגמה ב‫[example.js](https://github.com/3345SLMH/ip-router/blob/master/example.js)
     */
...
export function ipRouter (options?: ipRouterOptions): {
    /**
     ...
     * ---------------
     * מחזיר את הראוטר ‫(`ipRouter`) כפי שהוא - עם טייפ של ‫`Express.Router`, למניעת שגיאת טייפ
     * @example ```ts
     * import express from 'express';
     * import { ipRouter } from 'yemot-router';
     *  
     * const app = express();
...
export interface Call {
     ...
     * [פירוט נוסף על ‫read בתיעוד איי פי המשיח](https://f2.freeivr.co.il/post/78283)
    ...
    /**
     * מתודה להעברת השיחה למערכת אחרת באיי פי המשיח ללא עלות יחידות, באמצעות ‫"ראוטינג איי פי"<br>
     * הפונקציה מקבלת ארגומנט יחיד - סטרינג של מספר מערכת באיי פי להעברת השיחה אליה<br>
     */
     ...
     * במתודה זו יש להעביר את הסטרינג בדיוק כפי שמעוניינים שהשרת של איי פי יקבל אותו, והוא לא עובר אף ולידציה או עיבוד<br>
     * 
    ...
    /**
     * מכיל את כל הפרמטרים שנשלחו מאיי פי<br>
...
export interface TapOptions extends GeneralReadOptions {
     ...
     * פירוט על כל אופציה ניתן למצוא בתיעוד מודול API של איי פי המשיח, תחת"הערך השישי (הקשה)".
     * @default "No"
     */
